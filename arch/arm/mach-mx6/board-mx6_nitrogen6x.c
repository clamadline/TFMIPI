...
//~~C.LaMadline~~ 
#if defined(CONFIG_MXC_CAMERA_MT9M114_MIPI) || defined(CONFIG_MXC_CAMERA_MT9M114_MIPI_MODULE)
#define APTINA_CAMERA
#endif
...
#if defined(CSI0_CAMERA) || defined(MIPI_CAMERA) || defined(APTINA_CAMERA)
/*
...
#ifdef APTINA_CAMERA
struct pwm_device	*mipi_pwm;
static struct fsl_mxc_camera_platform_data mt9m114_mipi_data;
static int mt9m114_mipi_reset_active;

static void mt9m114_mipi_camera_io_init(void)
{
	IOMUX_SETUP(mipi_pads);
	...
	camera_reset(GP_MIPI_PWN, 1, GP_MIPI_RST, GP_MIPI_RST2);
	mt9m114_mipi_reset_active = 1;
	if (cpu_is_mx6dl()) {
		/*
		 * for mx6dl, mipi virtual channel 0 connect to csi0
		 * virtual channel 1 connect to csi1
		 */
		mxc_iomux_set_gpr_register(13, mt9m114_mipi_data.csi * 3, 3, mt9m114_mipi_data.csi);
	} else {
		if (mt9m114_mipi_data.csi == mt9m114_mipi_data.ipu) {
			/* select mipi IPU1 CSI0/ IPU2/CSI1 */
			mxc_iomux_set_gpr_register(1, 19 + mt9m114_mipi_data.csi, 1, 0);
		}
	}
}

static void mt9m114_mipi_camera_powerdown(int powerdown)
{
	pr_info("%s: powerdown=%d, power_gp=0x%x\n",
			__func__, powerdown, GP_MIPI_PWN);
	if (!IS_ERR(mipi_pwm)) {
	...
	if (!powerdown) {
		msleep(2);
		if (mt9m114_mipi_reset_active) {
			mt9m114_mipi_reset_active = 0;
			gpio_set_value(GP_MIPI_RST, 1);
			gpio_set_value(GP_MIPI_RST2, 1);
			msleep(20);
		}
...
static struct fsl_mxc_camera_platform_data mt9m114_mipi_data = {
	...
	.ipu = 0,
	.csi = 0,
	.io_init = mt9m114_mipi_camera_io_init,
	.pwdn = mt9m114_mipi_camera_powerdown,
	.lock = lock_i2c_3c_address,
	.unlock = unlock_i2c_3c_address,
};
...
#endif
#ifdef APTINA_CAMERA //~~C.LaMadline~~
	{
		I2C_BOARD_INFO("mt9m114_mipi", 0x48),
		.platform_data = (void *)&mt9m114_mipi_data,
	},
#endif
#ifdef TC358743_MIPI_CAMERA
	{
